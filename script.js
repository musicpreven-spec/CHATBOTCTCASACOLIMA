/* script.js - versión con logs para debug */

/* AVATARS: asegúrate de que existan en assets/ */
const AVATARS = {
...
function preloadImages(list){
  ...
  Object.values(list).forEach(src => {
    const img = new Image();
    img.src = src;
    promises.push(new Promise(res => { img.onload = () => { console.log('Preload OK', src); res(); }; img.onerror = () => { console.warn('Preload failed', src); res(); }; }));
  });
...
avatarImg.addEventListener('load', () => console.log('Avatar cargado:', avatarImg.src));
avatarImg.addEventListener('error', () => console.warn('Error cargando avatar:', avatarImg.src));

function scrollBottom(){ msgs.scrollTop = msgs.scrollHeight; }
function appendBot(html, opts = {}) { const bubble = document.createElement('div'); bubble.className = 'bubble bot'; bubble.innerHTML = html; if (opts.reaction) { const r = document.createElement('span'); r.className = 'reaction-pop'; r.textContent = opts.reaction; bubble.appendChild(r);} msgs.appendChild(bubble); scrollBottom(); }
function appendUser(text) { const bubble = document.createElement('div'); bubble.className = 'bubble user'; bubble.textContent = text; msgs.appendChild(bubble); scrollBottom(); }
function showThinking(){ const wrap = document.createElement('div'); wrap.className='bot-row'; const thinking = document.createElement('div'); thinking.className='thinking-bubble'; const dots = document.createElement('span'); dots.className='dots'; dots.innerHTML = '<span></span><span></span><span></span>'; thinking.appendChild(dots); wrap.appendChild(thinking); msgs.appendChild(wrap); scrollBottom(); return wrap; }
...
sendBtn.addEventListener('click', ()=> {
  const val = input.value.trim(); if(!val) return; appendUser(val); input.value=''; setAvatarExpression('blink', { effect:'flip' }); const thinking = showThinking();
  setTimeout(()=> {
    thinking.remove();
...
/* preload and start */
preloadImages(AVATARS).then(()=> { setAvatarExpression('idle'); startChat(); }).catch(()=> { setAvatarExpression('idle'); startChat(); });
